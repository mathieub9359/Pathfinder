...
DfsPath::DfsPath(std::vector<std::vector<Square>> newSquareGrid) : PathAlgo(newSquareGrid) {
    std::pair<int, int> startPos = findStartPos();

    squareGrid_[startPos.first][startPos.second].setVisited();
    squareStack_.push(squareGrid_[startPos.first][startPos.second]);
...
void DfsPath::visitNeighbours(const int row, const int col) {
    // Check top
    if(row - 1 >= 0 && squareGrid_[row - 1][col].getType() != Wall && !squareGrid_[row - 1][col].getVisited()) {
        ...
        squareStack_.push(squareGrid_[row - 1][col]);
    }
    ...
    if(col - 1 >= 0 && squareGrid_[row][col - 1].getType() != Wall && !squareGrid_[row][col - 1].getVisited()) {
        ...
        squareStack_.push(squareGrid_[row][col - 1]);
    }
...
std::pair<int, int> DfsPath::nextStepTraversal() {
    if(isFinishedAlgo()) {
        return {-1, -1};
    }

    Square currSquare = squareStack_.top();
